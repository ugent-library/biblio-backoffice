...
func (h *Handler) ConfirmWithdraw(w http.ResponseWriter, r *http.Request) {
	...
	dataset.ConfirmWithdraw(c, ctx.GetDataset(r), redirectUrl).Render(r.Context(), w)
}
...
func (h *Handler) Withdraw(w http.ResponseWriter, r *http.Request) {
	...
	if !c.User.CanWithdrawDataset(dataset) {
		h.Logger.Warnw("withdraw dataset: user has no permission to withdraw", "dataset", dataset.ID, "user", c.User.ID)
	...
	if err != nil {
		h.Logger.Errorf("withdraw dataset: could not save the dataset:", "errors", err, "dataset", dataset.ID, "user", c.User.ID)
