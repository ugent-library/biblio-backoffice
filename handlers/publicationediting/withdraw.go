...
func ConfirmWithdraw(w http.ResponseWriter, r *http.Request) {
	...
	views.ShowModal(publication.ConfirmWithdraw(c, ctx.GetPublication(r), redirectUrl)).Render(r.Context(), w)
}
...
func Withdraw(w http.ResponseWriter, r *http.Request) {
	...
	if !c.Repo.CanWithdrawPublication(c.User, publication) {
	...
	flash := flash.SimpleFlash().
		WithLevel("success").
		WithBody("<p>Publication was successfully withdrawn.</p>")
