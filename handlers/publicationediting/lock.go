...
func Lock(w http.ResponseWriter, r *http.Request) {
	...
	if errors.As(err, &conflict) {
		views.ErrorDialog(c, c.Loc.Get("publication.conflict_error_reload"), "").Render(r.Context(), w)
		return
	}
...
func Unlock(w http.ResponseWriter, r *http.Request) {
	...
	var conflict *snapstore.Conflict
	if errors.As(err, &conflict) {
		views.ErrorDialog(c, c.Loc.Get("publication.conflict_error"), r.URL.Query().Get("redirect-url")).Render(r.Context(), w)
		return
	}
