...
describe('Issue #1241: As a researcher or librarian, I should not be able to select an embargo date in the past - embargo switch does not happen because of that.', () => {
    ...
    it('should not be possible to select a date before tomorrow for embargo end date', () => {
      ...
      cy.ensureModal('Document details for file empty-pdf.pdf')
        .within(() => {
          ...
          cy.setFieldByLabel('Access level during embargo', 'UGent access - Local access')
          cy.setFieldByLabel('Access level after embargo', 'Public access - Open access')

          cy.get('#embargo_date').should('have.attr', 'min', tomorrow).type(today)
        })
      ...
      cy.ensureModal('Document details for file empty-pdf.pdf')
        .within(() => {
          cy.get('.alert.alert-danger')
            .should('be.visible')
            .should('contain.text', 'The embargo end date should be in the future')
          ...
          cy.get('#embargo_date')
            ...
            .should('be.visible')
            .should('have.text', 'The embargo end date should be in the future')
  ...
  describe('For datasets', () => {
    it('should not be possible to select a date before tomorrow for embargo end date', () => {
      ...
      cy.contains('.card-header', 'Dataset details').contains('.btn', 'Edit').click()
