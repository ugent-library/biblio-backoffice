...
export default function updateFields(
  ...
  const log = logCommand("updateFields", { callback, save }, section).snapshot(
    "before",
  );

  cy.location("pathname", NO_LOG).then((pathname) => {
    if (!pathname.match(/^\/(add-)?(publication|dataset).*$/)) {
      throw new Error(
    ...
    if ($lockLabel.length > 0) {
      throw new Error(
        "The updateFields command can only be called from an unlocked publication or dataset.",
      );
    }
  });
  ...
  cy.get(".nav.nav-tabs .nav-item .nav-link.active", NO_LOG).then(
    ($navLink) => {
      if ($navLink.length > 1) {
