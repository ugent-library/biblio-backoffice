...
export default function setUpPublication(publicationType: PublicationType, prepareForPublishing = false): void {
  ...
  cy.contains('Enter a publication manually', NO_LOG).find(':radio', NO_LOG).click(NO_LOG)
  ...
  cy.get('#show-content', NO_LOG)
    .attr('hx-get')
    .then(hxGet => {
      const biblioId = hxGet.match(/\/publication\/(?<biblioId>.*)\/description/)?.groups['biblioId']
...
declare global {
  namespace Cypress {
    interface Chainable {
      setUpPublication(publicationType: PublicationType, prepareForPublishing?: boolean): Chainable<void>
    }
  }
}
