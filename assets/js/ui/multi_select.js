...
export default function () {
    function setMultiSelect() {
        document.querySelectorAll(".custom-multi-select").forEach(function (el) {
            const iconicMultiSelectElement = new IconicMultiSelect({
                select: "#" + el.id,
            });
            ...
            iconicMultiSelectElement.subscribe(function (evt) {
                switch (evt.action) {
                  ...
                  case 'REMOVE_OPTION':
                    for (let i = 0; i < iconicMultiSelectElement._selectContainer.options.length; i++) {
                      if (iconicMultiSelectElement._selectContainer.options[i].value == evt.value) {
                        iconicMultiSelectElement._selectContainer.options[i].selected = false
                        iconicMultiSelectElement._selectContainer.options[i].removeAttribute("selected", "")
                      }
                     }
                    break;
                }
