...
func (c *DatasetPublications) Add(w http.ResponseWriter, r *http.Request) {
	...
	pub, err := c.Services.Store.GetPublication(pubID)
	if err != nil {
		log.Println(err)
	...
	publications, err := c.Services.Store.GetDatasetPublications(dataset)
	if err != nil {
		log.Println(err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
...
func (c *DatasetPublications) Remove(w http.ResponseWriter, r *http.Request) {
	...
	if err = c.Services.Store.RemovePublicationDataset(pub, dataset); err != nil {
		log.Println(err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
