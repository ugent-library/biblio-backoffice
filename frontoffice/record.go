...
func MapPublication(p *models.Publication, repo *repositories.Repo) *Record {
	...
	if p.ExternalFields != nil {
		...
		for _, fund := range []string{"bof", "iof"} {
			ecoomFund := ECOOMFund{}
			prefix := fmt.Sprintf("ecoom-%s-", fund)
			if vals, ok := p.ExternalFields.Get(fmt.Sprintf("%s%s", prefix, "css")); ok {
				ecoomFund.CSS = vals[0]
			}
			if vals, ok := p.ExternalFields.Get(fmt.Sprintf("%s%s", prefix, "weight")); ok {
				ecoomFund.Weight = vals[0]
			}
			if vals, ok := p.ExternalFields.Get(fmt.Sprintf("%s%s", prefix, "international-collaboration")); ok {
				b, _ := strconv.ParseBool(vals[0])
				ecoomFund.InternationalCollaboration = b
			}
