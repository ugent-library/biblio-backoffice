...
func getPersonByIdentifier(ctx context.Context, conn Conn, kind, value string, onlyActive bool) (*personRow, error) {
	var r personRow
...
func getActivePersonByUsername(ctx context.Context, conn Conn, username string) (*personRow, error) {
	...
	err := conn.QueryRow(ctx, getActivePersonByUsernameQuery, username).Scan(
		&r.ID,
		&r.Identifiers,
		&orgIDs,
		&r.Name,
