...
func journalArticleDetails(user *models.Person, loc *gotext.Locale, p *models.Publication) *display.Display {
	d := display.New().
		WithTheme("default").
		AddSection(
			&display.Text{
			...
			&display.Text{
				...
				Value: loc.Get("journal_article_types." + p.JournalArticleType),
			},
			&display.Link{
		...
		AddSection(
			...
			&display.Text{
				Label:    loc.Get("builder.journal_article.publication"),
				Value:    p.Publication,
				Required: p.ShowPublicationAsRequired(),
			},
			&display.Text{
				Label: loc.Get("builder.journal_article.publication_abbreviation"),
